<script lang="ts">
  ...
  import { mainMenuCategories } from "$lib/shared/links.js";
...
<Breadcrumb />
{#if slug !== "main"}
  {#each menu as { name, description, items }}
    <p>{name}</p>
    <p>{description}</p>
    <div class="grid grid-cols-1 gap-1 md:grid-cols-3 md:gap-3">
      {#each items as { name, isHot, prices }}
        <div>
  ...
  {/each}
{:else}
  {#each mainMenuCategories as { href, name }}
    <li>
      <a {href}>{name}</a>
    </li>
  {/each}
